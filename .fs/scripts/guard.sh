...
# Creds Cloudflare wymagane TYLKO gdy target ich potrzebuje (FS_NEEDS_CF=1).
# Target nie-CF (Expo, static...) deployuje bez konta CF.
if [ "${FS_NEEDS_CF:-1}" = 1 ]; then
  [ -f "$_ROOT/.env" ] || guard_die \
    "Brak .fs/.env. Uruchom .fs/setup.sh i podaj account_id + token SCOPED na jedno konto."
  ...
  printf "\033[1;32m✔ guard: target bez Cloudflare — deploy dozwolony\033[0m\n"
fi
